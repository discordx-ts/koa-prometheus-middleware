 ...
 *
 * @param path - url path.
 * @param  extraMasks - array of regexps which will replace
 * values in the URL.
 * @param [placeholder='#val'] - the placeholder that will replace id
 * like params in the url path.
 * @returns a normalized path, without ids.
 */
 ...
 *
 * @param status - status code of the requests
 * @returns the normalized status code.
 */
function normalizeStatusCode(status: number): "2XX" | "3XX" | "4XX" | "5XX" {
